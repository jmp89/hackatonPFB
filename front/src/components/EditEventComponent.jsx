    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const updatedFormDataEvent = {
                name: formDataEvent.name.length > 0 ? formDataEvent.name : placeholders.name,
                thematics: formDataEvent.thematics.length > 0 ? formDataEvent.thematics : (placeholders.thematics.reduce((x,y) => x+y, "")),
                technologies: formDataEvent.technologies.length > 0 ? formDataEvent.technologies : (placeholders.technologies.reduce((x,y) => x+y, "")),
                online_on_site: formDataEvent.online_on_site.length > 0 ? formDataEvent.online_on_site : placeholders.online_on_site,
                location: formDataEvent.location.length > 0 ? formDataEvent.location : placeholders.location,
                start_date: formDataEvent.start_date.length > 0 ? formDataEvent.start_date : placeholders.start_date,
                finish_date: formDataEvent.finish_date.length > 0 ? formDataEvent.finish_date : placeholders.finish_date,
                start_time: formDataEvent.start_time.length > 0 ? formDataEvent.start_time : placeholders.start_time,
                finish_time: formDataEvent.finish_time.length > 0 ? formDataEvent.finish_time : placeholders.finish_time,
                organizer: formDataEvent.organizer.length > 0 ? formDataEvent.organizer : placeholders.organizer,
                description: formDataEvent.description.length > 0 ? formDataEvent.description : placeholders.description
            };

            const themsArray = updatedFormDataEvent.thematics.split(",").map((them) => them.trim());
            const techsArray = updatedFormDataEvent.technologies.split(",").map((tech) => tech.trim());
            ...
            formDataFinal.append("online_on_site", updatedFormDataEvent.online_on_site);
            formDataFinal.append("location", updatedFormDataEvent.location);
            formDataFinal.append("start_date", updatedFormDataEvent.start_date.slice(0, 10));
            formDataFinal.append("finish_date", updatedFormDataEvent.finish_date.slice(0, 10));
            formDataFinal.append("start_time", updatedFormDataEvent.start_time);
            formDataFinal.append("finish_time", updatedFormDataEvent.finish_time);
            formDataFinal.append("organizer", updatedFormDataEvent.organizer);
            formDataFinal.append("description", updatedFormDataEvent.description);
        ...
        <>
            <main className="text-lg">
                ...
                <form className="mt-10 mx-auto flex flex-col bg-white px-6 rounded-t-lg shadow-md w-full max-w-3xl"
                    ...
                    <h2 className="mt-6 text-2xl font-bold text-center">Modificación de eventos</h2>

                    <section className="m-6 mb-10 flex flex-col">

                        <label htmlFor="name" className="mt-4 mb-2">
                            Nombre</label>
                        <input type="text" name="name" placeholder={placeholders?.name} value={formDataEvent.name} onChange={handleChange}
                            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black" />

                        <label htmlFor="thematics" className="mt-4 mb-2">
                                Temática/s </label>
                        <section>
                            <input type="text" name="thematics" placeholder={placeholders?.thematics.join(", ")}
                        ...
                        <label htmlFor="technologies" className="mt-4 mb-2">
                                Tecnología/s</label>
                        <section>
                            <input type="text" name="technologies" placeholder={placeholders?.technologies}
                        ...
                        <input type="text" name="online_on_site" placeholder={placeholders?.online_on_site} value={formDataEvent.online_on_site} onChange={handleChange}
                            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black" />

                        <label htmlFor="location" className="mt-4 mb-2">
                            Tendrá lugar en</label>
                        <input type="text" name="location" placeholder={placeholders?.location} value={formDataEvent.location} onChange={handleChange}
                            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black" />

                        <label htmlFor="start_date" className="mt-4 mb-2">
                            Fecha de inicio</label>
                        <input type="text" name="start_date" placeholder={placeholders?.start_date.slice(0, 10)} value={formDataEvent.start_date} onChange={handleChange}
                            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black" />

                        <label htmlFor="finish_date" className="mt-4 mb-2">
                            Fecha de finalización</label>
                        <input type="text" name="finish_date" placeholder={placeholders?.finish_date.slice(0, 10)} value={formDataEvent.finish_date} onChange={handleChange}
                            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black" />

                        <label htmlFor="start_time" className="mt-4 mb-2">
                            Hora de inicio</label>
                        <input type="text" name="start_time" placeholder={placeholders?.start_time} value={formDataEvent.start_time} onChange={handleChange}
                            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black" />

                        <label htmlFor="finish_time" className="mt-4 mb-2">
                            Hora de finalización</label>
                        <input type="text" name="finish_time" placeholder={placeholders?.finish_time} value={formDataEvent.finish_time} onChange={handleChange}
                            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black" />

                        <label htmlFor="organizer" className="mt-4 mb-2">
                            Organizador</label>
                        <input type="text" name="organizer" placeholder={placeholders?.organizer} value={formDataEvent.organizer} onChange={handleChange}
                            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black" />
                        ...
                        <textarea className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-black mb-2"    
                            name="description" placeholder={placeholders?.description} value={formDataEvent.description} onChange={handleChange} rows="10" />
                        ...
                        <button className="mt-8 mx-auto w-44 bg-black text-white py-2 rounded-lg font-bold text-lg hover:scale-105 transition-transform duration-300">Editar evento</button>
